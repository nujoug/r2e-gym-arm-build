...
test_39_install () {
    ...
    uv pip install setuptools "pytest<8" pytest-cov numpy 'pybids<0.7.0' wrapt mock appdirs
...
test_37_install () {
    ...
    uv pip install setuptools "pytest<8" pytest-cov numpy 'pybids<0.7.0' fasteners bids wrapt mock appdirs
    uv pip install -e .[full]
    uv pip install -e .[devel]
